...
with DAG(
    ...
    extract_load_task = PythonOperator(task_id="extract_load", python_callable=run_extract_load)

    # Task 3: Load to Postgres
    load_to_postgres_task = PythonOperator(task_id="load_to_postgres", python_callable=run_load_to_postgres)

    # Task 4: Transform with dbt
    transform = BashOperator(
        task_id="transform_dbt", bash_command=("dbt run --project-dir /opt/airflow/dbt_pink_flamingos --profiles-dir /opt/airflow")
    )
    ...
    hello_world >> extract_load_task >> load_to_postgres_task >> transform
